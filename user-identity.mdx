...
To initiate the User Identity Verification process, you need to first enable this feature within your chatbot settings. This can be done through the chatbot settings in "User Identity Verification" section.

Once the User Identity Verification is enabled your chatbot becomes equipped to automatically check for user tokens. This means each time a user attempts to interact with the chatbot, their identity token will be reviewed for authenticity.
...
2. **Configure for POST Requests:** Ensure that your API endpoint is configured to handle POST requests. This is crucial as the DONNAJAMES Easy will send token data via POST requests for verification.
...
1. **Token Submission:** Whenever a chat is rendered, DONNAJAMES Easy automatically sends a verification request to the provided API endpoint. This request includes the user's token.
...
To enhance the functionality of the User Identity Verification process and maintain records of user interactions, you have the option to save the data from successful verification responses.
...
## Step 4: Inserting Your Token into DONNAJAMES Easy Chatbot
...
<script src="https://donnajames.app/widget-asset.min.js" defer></script>
```
...
Append the **token** parameter to the **URL** in the src attribute, replacing **custom\_token** with the actual token value. Here's an example:

```html
<iframe
  src="https://donnajames.app/widget/162223v?token=<custom_token>"
  width="100%"
  height="500px"
  frameborder="0"
></iframe>
...
Congratulations! You have now successfully completed the setup of the User Identity Verification feature for your DONNAJAMES Easy Chatbot.
