...
On April 12, 2024, DONNAJAMES Easy introduced new subscriptions centered on Bring Your Own Key (BYOK). This feature allows eligible users to set up their own OpenAI API key inside their DONNAJAMES Easy accounts and enjoy unlimited Message Credits (MC). After you exhaust all MCs inside your DONNAJAMES Easy account, additional AI usage will be charged against your own API key and you will continue to be able to use AI features. New subscriptions do not come with monthly recurring MCs. However, you still have the option to purchase add-ons for monthly recurring MCs directly from DONNAJAMES Easy.

## Setting up BYOK

To set up your own OpenAI API key, you will first need to apply for one directly from OpenAI's website. Sign up for an account on OpenAI, then go to this link: [https://platform.openai.com/account/api-keys.](https://platform.openai.com/account/api-keys.) Additional instructions for obtaining an API key
can be found here: [https://help.openai.com/en/articles/4936850-where-do-i-find-my-openai-api-key.](https://help.openai.com/en/articles/4936850-where-do-i-find-my-openai-api-key.)

After you obtain your API key, store it in a **private and secure** location. DONNAJAMES Easy recommends that you use a separate API key for each of your BYOK applications (including DONNAJAMES Easy), and to never share your API key with anyone else. Next, navigate
to DONNAJAMES Easy. After logging in, go to the top right corner of the UI. Click the "person" icon the bring up a dropdown menu.
...
There are many parts to a LLM query that DONNAJAMES Easy submits to LLM provider for processing. At a high level, they can be broken down
...
DONNAJAMES Easy supports a variety of OpenAI LLMs, as well as different versions of the same LLM with custom token limit cutoffs.
...
<Note>
  As of October 2024, DONNAJAMES Easy still uses gpt-3.5-turbo-0613 for GPT-3.5. This
...
own API key. This is independent of whether your users have supplied their API key for their personal accounts. Since the official DONNAJAMES Easy
