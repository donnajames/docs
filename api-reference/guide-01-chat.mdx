...
In this section, we will guide you on how to start chatting with your chatbot using the DONNAJAMES. Easy API. Before you begin, please ensure you meet the following prerequisites:

## Prerequisites

* An API key for accessing the DONNAJAMES. Easy API.
...
To create a chat session, you need to use a POST request to the API endpoint: `https://donnajames.app/api/v1/chatbot/{chatbot_uuid}/session/create`.
...
Here're example command to create a chatbot using the DONNAJAMES. Easy API:

<Warning>Replace **token** with your actual API key.</Warning>

<CodeGroup>
  ```bash Curl
  curl --location --request POST 'https://donnajames.app/api/v1/chatbot/{chatbot_uuid}/session/create' \
...
`https://donnajames.app/api/v1/session/{session_uuid}/message/stream`
...
Here're example command to create a message using the API:

<Warning>Replace **token** with your actual API key.</Warning>

<CodeGroup>
  ```bash Curl
  curl --location --request POST 'https://donnajames.app/api/v1/session/{session_uuid}/message/stream' \
  --header 'Content-Type: application/json' \
...
That's it! You've now learned how to chat with your own chatbot using the API.