...
In this guide, we will walk through the process of uploading data sources to your chatbot using the API.
...
The API endpoint for uploading data sources to your chatbot is: `https://donnajames.app/api/v1/chatbot/{uuid}/data-source/url`.
...
Here're example command sto create a chatbot using the API:

<Warning>Replace **token** with your actual API key.</Warning>

<CodeGroup>
  ```bash Curl
  curl --location --request POST 'https://donnajames.app/api/v1/chatbot/{uuid}/data-source/url' \
...
That's it! You've now learned how to upload data sources to your chatbot using the API.