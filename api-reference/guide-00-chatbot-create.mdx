...
1. An API key for accessing the DONNAJAMES. Easy API.
...
```
https://donnajames.app/api/v1/chatbot/create
```
...
* rate\_limit (array, optional): Set the rate limit for the chatbot in messages per minute. It's an array with two values: \[20, 240].First number: amount of messages. Min `1` - Max `100`
...
Here're example command sto create a chatbot using the DONNAJAMES. Easy API:

<Warning>Replace **token** with your actual API key.</Warning>

<CodeGroup>
  ```bash Curl
  curl --location --request POST 'https://donnajames.app/api/v1/chatbot/create' \
...
That's it! You've now learned how to create your own chatbot using the DONNAJAMES. Easy API.
...
```
https://donnajames.app/api/v1/chatbot/{chatbot_uuid}/agents
```
...
```
https://donnajames.app/api/v1/agent/{uuid}/update
```
...
<CodeGroup>
  ```bash Curl
  curl --location --request POST 'https://donnajames.app/api/v1/agent/{uuid}/update' \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer <token>' \
  --data-raw '{
